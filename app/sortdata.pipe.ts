...
/**
  ...
  static caseInsensitiveSort(a: any, b: any) {
    if (SortdataPipe.isString(a) && SortdataPipe.isString(b)) {
      return a.localeCompare(b);
    }
    return SortdataPipe.defaultCompare(a, b);
  }
  ...
  private sortArray(value: any[], expression?: any, reverse?: boolean, isCaseInsensitive?: boolean, comparator?: Function): any[] {
    ...
    if (isDeepLink) {
      expression = SortdataPipe.parseExpression(expression);
    }
    ...
    if (comparator && typeof comparator === 'function') {
      ...
      compareFn = isCaseInsensitive ? SortdataPipe.caseInsensitiveSort : SortdataPipe.defaultCompare;
    }
    ...
    const array: any[] = value.sort((a: any, b: any): number => {
      ...
      return compareFn(SortdataPipe.getValue(a, expression), SortdataPipe.getValue(b, expression));
    });
  ...
  private transformObject(
    ...
    const parsedExpression = SortdataPipe.parseExpression(expression);
    let lastPredicate = parsedExpression.pop();
    let oldValue = SortdataPipe.getValue(value, parsedExpression);

    if (!Array.isArray(oldValue)) {
      parsedExpression.push(lastPredicate);
      lastPredicate = null;
      oldValue = SortdataPipe.getValue(value, parsedExpression);
    }
    ...
    SortdataPipe.setValue(value, this.transform(oldValue, lastPredicate, reverse, isCaseInsensitive), parsedExpression);
    return value;
  }
